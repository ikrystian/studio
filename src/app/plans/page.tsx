...
interface TrainingPlan {
  ...
  name: string;
  description: string;
  goal: string; // e.g., "Budowa siły", "Redukcja tkanki tłuszczowej", "Poprawa kondycji"
  duration: string; // e.g., "4 tygodnie", "8 tygodni", "3 miesiące"
...
const MOCK_TRAINING_PLANS: TrainingPlan[] = [
  ...
  },
  {
    ...
    duration: '4 tygodnie',
    icon: StretchHorizontal, // Assuming StretchHorizontal exists or we can find a similar one
  },
  {
    id: 'plan4',
...
const PLAN_GOALS = ["Wszystkie", "Budowa podstawowej siły i masy mięśniowej", "Redukcja tkanki tłuszczowej i poprawa kondycji", "Poprawa elastyczności i mobilności", "Utrzymanie formy i wszechstronny rozwój"];

export default function TrainingPlansPage() {
  const [searchTerm, setSearchTerm] = React.useState('');
  const [selectedGoal, setSelectedGoal] = React.useState('Wszystkie');

  const filteredPlans = MOCK_TRAINING_PLANS.filter(plan => {
    const matchesSearch = plan.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          plan.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesGoal = selectedGoal === 'Wszystkie' || plan.goal === selectedGoal;
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="container mx-auto">
          <section className="mb-8">
            <h2 className="text-3xl font-semibold tracking-tight mb-2">Odkryj Plany Treningowe</h2>
          ...
          {filteredPlans.length > 0 ? (
            ...
            <div className="mt-10 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-muted-foreground mb-4">
                {searchTerm || selectedGoal !== 'Wszystkie' ? `Nie znaleziono planów pasujących do Twoich kryteriów.` : "Nie utworzono jeszcze żadnych planów treningowych."}
              </p>
