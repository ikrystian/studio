...
import {
  MOCK_AVAILABLE_WORKOUTS_FOR_PLAN_EDITOR,
  MOCK_DAY_TEMPLATES_FOR_PLAN_EDITOR,
} from "@/lib/mockData";

// DATABASE INTEGRATION:
// - Initial Data: Plan details are fetched from the SQLite database via API endpoint.
// - Available Workouts/Templates: Still loaded from mock data (can be migrated to database later).
// - "Quick Create Workout": Adds to an in-memory `availableWorkouts` state, simulating an update to a global workout DB.
// - Saving Plan: Updates the plan in the SQLite database via API endpoint.
...
const SelectWorkoutDialog = dynamic(
  () =>
    import("@/components/plans/select-workout-dialog").then(
      (mod) => mod.SelectWorkoutDialog
    ),
  {
    loading: () => (
...
const QuickCreateWorkoutDialog = dynamic(
  () =>
    import("@/components/plans/quick-create-workout-dialog").then(
      (mod) => mod.QuickCreateWorkoutDialog
    ),
  {
    loading: () => (
      <div className="fixed inset-0 bg-background/50 flex items-center justify-center z-50">
  ...
  .refine(
    (data) => {
      return data.days.some(
        (day) => day.assignedWorkoutId || day.templateId || day.isRestDay
      );
    },
    {
      message:
        "Plan musi zawierać przynajmniej jeden przypisany trening, szablon lub dzień odpoczynku.",
      path: ["days"],
    }
  );
...
export default function EditTrainingPlanPage() {
  ...
  const [initialPlanData, setInitialPlanData] =
    React.useState<DetailedTrainingPlan | null>(null);

  const [isWorkoutSelectionDialogOpen, setIsWorkoutSelectionDialogOpen] =
    React.useState(false);
  const [isQuickCreateWorkoutDialogOpen, setIsQuickCreateWorkoutDialogOpen] =
    React.useState(false);
  const [currentDayIndexToAssign, setCurrentDayIndexToAssign] = React.useState<
    number | null
  >(null);
  ...
  const [copiedDayConfig, setCopiedDayConfig] = React.useState<Omit<
    PlanDayFormValues,
    "dayName"
  > | null>(null);
    ...
    const fetchPlanData = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          setServerError(
            data.message || "Plan treningowy nie został znaleziony"
          );
          toast({
            title: "Błąd",
            description:
  ...
  const [calendarViewMonth, setCalendarViewMonth] = React.useState(
    watchedStartDate || new Date()
  );
  ...
  React.useEffect(() => {
    if (watchedStartDate && !isSameMonth(watchedStartDate, calendarViewMonth)) {
      ...
      !watchedStartDate &&
      !isSameMonth(new Date(), calendarViewMonth)
    ) {
  ...
  const handleQuickWorkoutCreated = (
    newWorkoutData: QuickCreateWorkoutFormData
  ) => {
  ...
  const handleCopyDay = (sourceDayIndex: number) => {
    ...
    toast({
      ...
      description: `Konfiguracja dnia "${dayName}" skopiowana. Wybierz dzień docelowy.`,
    });
  };
    ...
    try {
      ...
      if (data.success) {
        toast({
          ...
          description: `Plan "${values.planName}" został pomyślnie zaktualizowany.`,
        });
        router.push(`/dashboard/plans/${planId}`);
      } else {
        setServerError(data.message || "Błąd podczas aktualizacji planu");
        toast({
          title: "Błąd aktualizacji",
          description:
            data.message || "Nie udało się zaktualizować planu treningowego.",
          variant: "destructive",
        });
      }
    } catch (err) {
      const errorMessage = "Błąd połączenia podczas aktualizacji planu";
      setServerError(errorMessage);
      toast({
        title: "Błąd połączenia",
        description: errorMessage,
  ...
  const getDayAssignment = (date: Date) => {
    ...
    if (!planStartDate || isBefore(date, startOfMonth(planStartDate))) {
      // Use startOfMonth for consistency
      return null;
    }
    const diff = differenceInCalendarDays(date, startOfMonth(planStartDate)); // Use startOfMonth for consistency
    const dayIndexInCycle = diff % 7;
    return form.watch(`days.${dayIndexInCycle}`);
  };

  if (pageIsLoading) {
    // return <EditTrainingPlanPageSkeleton />; // Removed for no-skeleton approach
    return (
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background text-foreground">
        <Alert variant="destructive" className="max-w-lg">
          ...
          <AlertDescription>
            Nie udało się załadować danych planu do edycji. Sprawdź, czy ID
            planu jest poprawne lub spróbuj ponownie.
            ...
            >
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Zmodyfikuj nazwę, opis i inne kluczowe dane dla swojego
                    planu.
                  </CardDescription>
                </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Np. Mój Plan Siłowy na Masę"
                            {...field}
                            disabled={formIsLoading}
                          />
                        </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Krótki opis celów, metodologii, itp."
                            {...field}
                            disabled={formIsLoading}
                          />
                        </FormControl>
                            ...
                            <PopoverContent className="w-auto p-0">
                              <Calendar
                                ...
                                onSelect={(date) => {
                                  field.onChange(date);
                                  if (date)
                                    setCalendarViewMonth(startOfMonth(date));
                                }}
                                disabled={(date) =>
                                  date <
                            ...
                            <PopoverTrigger asChild>
                              <FormControl>
                                <Button
                                  ...
                                  disabled={
                                    formIsLoading || !form.watch("startDate")
                                  }
                                >
                                  <CalendarDays className="mr-2 h-4 w-4" />
                                  {field.value ? (
                          ...
                          <span className="text-purple-600 dark:text-purple-400 flex items-center">
                            ...
                            {day.assignedWorkoutName &&
                              day.assignedWorkoutId !==
                                MOCK_DAY_TEMPLATES_FOR_PLAN_EDITOR.find(
                                  (t) => t.id === day.templateId
                                )?.assignedWorkoutId && (
                                <span className="text-xs text-muted-foreground ml-1">
                                  (zastąpiony: {day.assignedWorkoutName})
                                </span>
                              )}
                          </span>
                        ) : day.isRestDay ? (
                          <span className="text-green-600 flex items-center">
                            <Coffee className="mr-2 h-5 w-5" /> Dzień Odpoczynku
                          </span>
                        ) : day.assignedWorkoutName ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {isPastingModeActive ? (
                          <Button
                            ...
                            disabled={formIsLoading}
                          >
                            <ClipboardPaste className="mr-2 h-4 w-4" /> Wklej
                            konfigurację tutaj
                          </Button>
                        ) : (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              <DropdownMenuItem
                                onClick={() =>
                                  handleOpenWorkoutSelectionDialog(index)
                                }
                              >
                                <PlusCircle className="mr-2 h-4 w-4" /> Wybierz
                                Istniejący Trening
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() =>
                                  handleOpenQuickCreateDialog(index)
                                }
                              >
                                <Edit3 className="mr-2 h-4 w-4" /> Szybkie
                                Stwórz Nowy Trening
                              </DropdownMenuItem>
                              <DropdownMenuSub>
                                <DropdownMenuSubTrigger>
                                  <LayoutDashboard className="mr-2 h-4 w-4" />{" "}
                                  Przypisz Szablon Dnia
                                </DropdownMenuSubTrigger>
                                <DropdownMenuSubContent>
                                  {MOCK_DAY_TEMPLATES_FOR_PLAN_EDITOR.map(
                                    (template) => (
                                      <DropdownMenuItem
                                        key={template.id}
                                        onClick={() =>
                                          handleAssignTemplate(index, template)
                                        }
                                      >
                                        {template.name}
                                      </DropdownMenuItem>
                                  ...
                                  {MOCK_DAY_TEMPLATES_FOR_PLAN_EDITOR.length ===
                                    0 && (
                              ...
                              {!day.isRestDay && (
                                ...
                                >
                                  <Coffee className="mr-2 h-4 w-4" /> Oznacz
                ...
                <CardHeader>
                  ...
                  <CardDescription>
                    {form.watch("startDate")
                      ? "Zobacz, jak Twój 7-dniowy cykl przekłada się na kalendarz."
                      : "Wybierz datę rozpoczęcia planu, aby zobaczyć wizualizację w kalendarzu."}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {form.watch("startDate") ? (
                    <>
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <h3 className="text-lg font-semibold">
                          {format(calendarViewMonth, "LLLL yyyy", {
                            locale: pl,
                          })}
                        </h3>
                        <Button
                      ...
                      <div className="grid grid-cols-7 gap-px border-l border-t bg-border">
                        ...
                        {daysBeforeMonth.map((_, i) => (
                          <div
                            key={`empty-start-${i}`}
                            className="bg-muted/30 border-b border-r min-h-[80px]"
                          ></div>
                        ))}
                        {daysInMonth.map((day) => {
                          const assignment = getDayAssignment(day);
                          const isDayBeforeStartDate = isBefore(
                            day,
                            startOfMonth(form.watch("startDate")!)
                          );
                          let content = null;
                        ...
                        {Array.from({
                          length:
                            (7 -
                              ((daysBeforeMonth.length + daysInMonth.length) %
                                7)) %
                            7,
                        }).map((_, i) => (
                          <div
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Aby zobaczyć wizualizację planu w kalendarzu, wybierz
                        najpierw "Datę Rozpoczęcia Planu" w sekcji powyżej.
                      </AlertDescription>
                    </Alert>
                  )}
                </CardContent>
              ...
              <div className="flex justify-end space-x-4">
                {!isPastingModeActive && (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => router.push(`/dashboard/plans/${planId}`)}
                    disabled={formIsLoading}
                  >
                    Anuluj
                  </Button>
