...
import { format, parseISO, isValid as isDateValid } from "date-fns"; // Renamed isValid to isDateValid
...
interface AutoSavedWorkoutState {
  ...
  elapsedTime: number;
  // exercises?: ExerciseInWorkout[]; // Consider adding if needed for robust resume
}
...
export default function ActiveWorkoutPage() {
  ...
  React.useEffect(() => {
    ...
    // Try to load custom pending workout first
    ...
    // If not a custom pending workout, try to load autosave
      ...
      try {
        ...
        if (autoSavedDataString) {
          const parsedData: AutoSavedWorkoutState = JSON.parse(autoSavedDataString);
           if (parsedData.workoutId === workoutId || workoutId.startsWith("custom-repeat-")) { // Ensure it's for the current workout or a resumed repeat
            setSavedWorkoutData(parsedData);
            setShowResumeDialog(true);
             // Don't set workoutDataLoaded to true yet, user needs to decide.
             // setIsLoading(false) will be handled in handleResumeWorkout
            return; // Stop further loading until user decides
           } else {
             localStorage.removeItem(ACTIVE_WORKOUT_AUTOSAVE_KEY); // Stale autosave for different workoutId
           }
        }
      } catch (error) { console.error("Error loading autosaved data:", error); localStorage.removeItem(ACTIVE_WORKOUT_AUTOSAVE_KEY); }
    }

    // If no autosave dialog, or if not a custom workout, load from MOCK_WORKOUTS
  ...
  const handleResumeWorkout = React.useCallback((resume: boolean) => {
    if (resume && savedWorkoutData) {
      ...
      // If currentWorkout is not set yet, or if the savedWorkoutData's ID is different, try to set it.
      // This covers resuming a standard mock workout.
      if (!currentWorkout || (baseWorkout && currentWorkout.id !== baseWorkout.id)) {
        setCurrentWorkout(baseWorkout || null); // Set to null if baseWorkout not found
      }
      // For custom-repeat workouts, currentWorkout should already be set from the initial load logic
      // If not, it means PENDING_CUSTOM_WORKOUT_KEY was cleared or mismatch, which is an edge case.
      // We trust savedWorkoutData.workoutName for the display if currentWorkout is minimal/missing.
      ...
      if (savedWorkoutData.workoutStartTime && isDateValid(parseISO(savedWorkoutData.workoutStartTime))) {
        ...
        setWorkoutStartTime(new Date()); // Fallback if saved time is invalid
      }
      ...
      } else if (currentWorkout && !workoutStartTime) { // Custom repeat was discarded, but structure might be in currentWorkout
    ...
    setIsLoading(false);
  }, [savedWorkoutData, ACTIVE_WORKOUT_AUTOSAVE_KEY, toast, workoutId, currentWorkout, workoutStartTime]);
  ...
  React.useEffect(() => {
    if (!isLoading && currentWorkout && workoutStartTime && ACTIVE_WORKOUT_AUTOSAVE_KEY) {
      const autoSaveData: AutoSavedWorkoutState = {
        ...
        elapsedTime,
        // Consider adding currentWorkout.exercises if they might be dynamically generated
      };
    ...
    if (currentExercise && !isLoading && !showResumeDialog) {
        const getSuggestion = (
            ...
            if (relevantSessions.length > 0) {
                ...
                const lastSet = lastSession.setsPerformed[0]; // Simplification: considering first set of last session
                if (lastSet) {
