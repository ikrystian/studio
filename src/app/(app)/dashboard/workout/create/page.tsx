...
const ExerciseSelectionDialog = dynamic(
  () =>
    import("@/components/workout/exercise-selection-dialog").then(
      (mod) => mod.ExerciseSelectionDialog
    ),
  {
    loading: () => (
...
const QuickAddExerciseDialog = dynamic(
  () =>
    import("@/components/workout/quick-add-exercise-dialog").then(
      (mod) => mod.QuickAddExerciseDialog
    ),
  {
    loading: () => (
      <div className="fixed inset-0 bg-background/50 flex items-center justify-center z-50">
...
const exerciseInWorkoutSchema = z.object({
  ...
  sets: z.union([
    z.coerce
      .number({ invalid_type_error: "Serie muszą być liczbą." })
      .positive("Liczba serii musi być dodatnia.")
      .optional()
      .nullable(),
    z.literal(""),
  ]),
  reps: z.string().optional().nullable(),
  restTimeSeconds: z.union([
    z.coerce
      .number({ invalid_type_error: "Czas odpoczynku musi być liczbą." })
...
export type ExerciseInWorkoutFormValues = z.infer<
  typeof exerciseInWorkoutSchema
>;
...
const workoutFormSchema = z.object({
  ...
  exercises: z
    .array(exerciseInWorkoutSchema)
    .min(1, "Trening musi zawierać przynajmniej jedno ćwiczenie."),
});
...
export default function CreateWorkoutPage() {
  ...
  const [exerciseFetchError, setExerciseFetchError] = React.useState<
    string | null
  >(null);

  const [isExerciseSelectionDialogOpen, setIsExerciseSelectionDialogOpen] =
    React.useState(false);
  const [isQuickAddExerciseDialogOpen, setIsQuickAddExerciseDialogOpen] =
    React.useState(false);

  const [masterExerciseList, setMasterExerciseList] = React.useState<
    ...
    async function fetchExercises() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message ||
              `Nie udało się załadować listy ćwiczeń. Status: ${response.status}`
          );
        }
        const data = await response.json();
        if (data.success) {
          setMasterExerciseList(data.exercises);
        } else {
          throw new Error(
            data.message ||
              "Nie udało się załadować ćwiczeń (odpowiedź API bez sukcesu)."
          );
        }
      } catch (error: any) {
        console.error("Error fetching exercises for create page:", error);
        setExerciseFetchError(
          error.message || "Wystąpił nieznany błąd podczas ładowania ćwiczeń."
        );
        setMasterExerciseList([]);
        toast({
          title: "Błąd Ładowania Ćwiczeń",
  ...
  const handleOpenExerciseSelectionDialog = () => {
    if (exerciseFetchError) {
      toast({
        title: "Błąd Ładowania Ćwiczeń",
        description:
          "Nie można otworzyć okna wyboru, ponieważ lista ćwiczeń nie została załadowana.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleExercisesSelected = (
    ...
    const exercisesToAppend = selectedExercises.map((ex) => ({
      id: ex.id,
      name: ex.name,
      sets: null, // Changed from "" to null
      reps: "",
      restTimeSeconds: null, // Changed from "" to null
      targetRpe: "",
      exerciseNotes: "",
    }));
    ...
    if (
      ...
      typeof form.formState.errors.exercises.message === "string"
    ) {
  ...
  const handleQuickExerciseCreated = (
    newDbExercise: SelectableExerciseType
  ) => {
    setMasterExerciseList((prev) => [...prev, newDbExercise]); // Add to client-side master list
    append({
      // Append to current workout form
      id: newDbExercise.id,
  ...
  const handleCopyFromPrevious = (index: number) => {
    ...
    form.setValue(
      ...
      previousExerciseData.restTimeSeconds || ""
    );
    form.setValue(
      `exercises.${index}.targetRpe`,
      previousExerciseData.targetRpe || ""
    );
    form.setValue(
      `exercises.${index}.exerciseNotes`,
      previousExerciseData.exerciseNotes || ""
    );
    toast({
  ...
  const handleLoadMockDefaults = (index: number) => {
    ...
    form.setValue(
      `exercises.${index}.exerciseNotes`,
      "Załadowano domyślne parametry (symulacja)."
    );
    toast({
      title: "Domyślne Parametry Załadowane (Symulacja)",
      description: `Ustawiono domyślne wartości dla ćwiczenia: ${form.getValues(
  ...
  async function onSubmit(values: WorkoutFormValues) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setServerError(
          result.message || "Wystąpił błąd podczas zapisywania treningu."
        );
        toast({
          title: "Błąd Zapisu",
        ...
        <div className="container mx-auto flex h-14 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={handleOpenQuickAddExerciseDialog}
              disabled={isLoading}
            >
            ...
            >
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Nadaj swojemu nowemu treningowi nazwę i opcjonalnie określ
                    jego typ.
                  </CardDescription>
                </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Np. Mój Trening Siłowy Poniedziałek"
                            {...field}
                            disabled={isLoading}
                          />
                        </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Określ główny charakter tego treningu.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Dodaj ćwiczenia i skonfiguruj ich parametry.
                    </CardDescription>
                  </div>
                  <Button
                    type="button"
                    variant="outline"
                    onClick={handleOpenExerciseSelectionDialog}
                    disabled={isLoading || !!exerciseFetchError}
                  >
                ...
                <CardContent>
                  {exerciseFetchError && (
                    <Alert variant="destructive" className="mb-4">
                      ...
                      <AlertDescription>
                        {exerciseFetchError} Spróbuj odświeżyć stronę. Nie można
                        dodać ćwiczeń z bazy. Możesz skorzystać z opcji "Szybkie
                        Dodaj Ćwiczenie".
                      </AlertDescription>
                    </Alert>
                  )}
                  {fields.length === 0 ? (
                    <div className="py-6 text-center text-muted-foreground">
                      ...
                      <p>
                        Kliknij "+ Dodaj z Bazy" lub "Szybkie Dodaj Ćwiczenie",
                        aby rozpocząć.
                      </p>
                    </div>
                  ) : (
                            ...
                            <div className="flex items-start justify-between gap-2 mb-4">
                              ...
                              <div className="flex items-center space-x-1 flex-shrink-0">
                                ...
                                <Button
                                  ...
                                  disabled={
                                    index === fields.length - 1 || isLoading
                                  }
                            ...
                            <div className="flex items-center gap-2 mb-4">
                              {index > 0 && (
                                <Button
                                  ...
                                  className="text-xs"
                                >
                                  <Copy className="mr-1 h-3 w-3" /> Kopiuj z
                                  pop.
                                </Button>
                              )}
                              <Button
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    ...
                                    <FormControl>
                                      <Textarea
                                        rows={2}
                                        placeholder="Np. skup się na technice, wolna faza negatywna"
                                        {...field}
                                        value={
                                          field.value === null ||
                                          field.value === undefined
                  ...
                  {form.formState.errors.exercises &&
                    typeof form.formState.errors.exercises.message ===
                      "string" && (
